...
###### The common features of an IDE consist of many helpful features such as Syntax highlighting, Bracket Matching, Step Over/Into/Out and Auto-Completion. All of these are used in many different IDEs they all do the same job within the certain IDEs but it’s how the user decides to use them and how it can benefit the user 

###### The first feature is Syntax Highlighting. What syntax highlighting does is it makes the code different colours and fonts according to the category of terms. So you can easily Identify a function or an if statement and many more different coding standards. This can benefit the user as it is now easier to find what he may want also it can help to spot a problem is something that is meant to be highlighted is not highlighting. The advantages of this is it makes your code easier to understand as people can see the main parts of the program, also as the user you are able to spot problems if they were to occur and something was not to highlight another advantage of syntax highlighting is that is makes it easier to spot errors.

###### The second feature is Bracket Matching which is in almost every IDE instead of you having to press both brackets it will close it for you. So you don’t forget because the code will not run if there is not a closed bracket. This feature is simply to help out the user so they don’t forget to close their brackets and break their code. It is automatic feature that is done when you open your brackets. Bracket matching is particularly useful when many nested if statements and program loops are involved. Because you can easily spot which bracket goes with each one by hovering over it or clicking it so you can see which code is inside them certain brackets. The benefits of Bracket matching are firstly your code should always have a closed bracket thanks to bracket matching. Secondly you can see which code is indented to certain parts of the code when using bracket matching. It also makes the code easier to understand when someone else is looking through your code.

###### The Third common feature is Step Over/Into/Out. Starting with Step into its job is to Executes code one statement at a time. Helping you find the errors as you work through your work. Step Over is similar to into but Step Over executes the procedure as a unit, and then steps to the next statement in the current procedure. Finally, Step Out executes the remaining lines of a function in which the current execution point lies one problem with Step Out is that the code has to be in break mode only. All of these are helpful to do one thing and that is to Debug your code and to make sure that it will run there is just many different ways you can debug your code these three are only some of many but they are probably most used. The advantages of using Step Over/Into/Out are that you have many different options when it comes to debugging and some ways suit certain parts of code better and with Step Over/Into/Out you have three different options to choose from or even try all three. 

###### Finally, Auto-Completion is designed to save time while writing code. As you start to type the first part of a function, it suggests or completes the function and any arguments or variables. If you are trying to link certain variables, then it will normally give you options as a drop down to see if you want to select it instead of trying out the whole name if it is long. Auto-Completion appears in many program environments, an example implementation being Visual Studio's IntelliSense. The term was originally popularized as picklist. Code-Completion works using an automatically generated in-memory database of classes, variable names, and other constructs that the application being edited defines or references. The advantages of Auto-Completion are, firstly it can save the coder time. Secondly it can simplify linking variables and functions as it will only list ones that are available so if one isn’t then something is wrong.  
